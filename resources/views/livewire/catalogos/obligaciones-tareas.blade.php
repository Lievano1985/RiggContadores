{{-- 
Vista: Catálogo de Obligaciones y Tareas
...
<div x-data="{ openSidebar: @entangle('sidebarVisible') }">
    ...
    <div class="flex items-center justify-between mb-4">
        <h2 class="text-xl font-bold text-stone-600 dark:text-white">
            Catálogo de Obligaciones y Tareas
        </h2>
    ...
    {{-- BUSCADOR --}}
    <div class="mb-4">
        <label class="block text-xs font-semibold text-stone-600 dark:text-white mb-1">
            Filtro (obligación o tarea)
        </label>
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/60">
                        ...
                        <td class="px-4 py-2">
                            ...
                            <div class="text-xs text-gray-500 dark:text-gray-400">
                                @if ($esUnica)
                                    Única / eventual
                                @else
                                    Mes inicio: {{ $obligacion->mes_inicio }},
                                    Desfase: {{ $obligacion->desfase_meses }},
                                    Día corte: {{ $obligacion->dia_corte }}
                                @endif
                            </div>
                        </td>
                        ...
                        <td class="px-4 py-2 text-right space-x-2">
                            <button wire:click="abrirCrearTarea({{ $obligacion->id }})"
                                class="px-2 py-1 text-xs bg-amber-100 dark:bg-amber-900/40 rounded">
                                + Tarea
                            </button>
                            ...
                            <button wire:click="eliminarObligacion({{ $obligacion->id }})"
                                ...
                                class="px-2 py-1 text-xs border border-red-400 text-red-600 rounded">
                                Eliminar
                            </button>
                        </td>
                    </tr>
    ...
    <div x-cloak x-show="openSidebar" x-transition.opacity class="fixed inset-0 z-40 flex justify-end bg-black/40 ">
        ...
        <div x-show="openSidebar" x-transition
            ...
            <div class="px-4 py-3 border-t flex justify-end space-x-3">
                ...
                <button @click="$wire.cerrarSidebar()" class="px-3 py-2 bg-gray-200 rounded">
                    Cancelar
                </button>

                {{-- Guardar --}}
                @if (in_array($sidebarModo, ['crear_obligacion', 'editar_obligacion']))
                    <button wire:click="guardarObligacion" class="px-4 py-2 bg-amber-600 text-white rounded">
                        Guardar
                    </button>
                @endif

                @if (in_array($sidebarModo, ['crear_tarea', 'editar_tarea']))
                    <button wire:click="guardarTarea" class="px-4 py-2 bg-amber-600 text-white rounded">
                        Guardar
                    </button>
                @endif
