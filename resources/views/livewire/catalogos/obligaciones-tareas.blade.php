...
<div x-data="{ openSidebar: @entangle('sidebarVisible') }"> 
    ...
    <div class="flex items-center justify-between mb-4">
        ...
        <button
            wire:click="abrirCrearObligacion"
            class="px-4 py-2 bg-amber-600 text-white rounded hover:bg-amber-700 transition"> + Nueva obligación </button>
    </div> 

    {{-- BUSCADOR --}}
    <div class="mb-4"> 
        <label class="block text-xs font-semibold text-stone-600 dark:text-white mb-1"> Filtro (obligación o tarea) </label>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @forelse ($obligaciones as $obligacion) 
                    @php
                        ...
                        $esUnica = in_array(strtolower($obligacion->periodicidad), ['unica', 'única', 'eventual'], true);
                    @endphp
                    ...
                    @if ($expandida)
                        <tr class="bg-gray-50/60 dark:bg-gray-900/40">
                            <td colspan="7" class="px-6 pb-4 pt-1">
                                ...
                                @else
                                    <div class="border border-gray-200 dark:border-gray-700 rounded-lg mt-2 overflow-hidden">
                                            ...
                                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                                @foreach ($obligacion->tareasCatalogo as $tarea)
                                                    <tr class="hover:bg-white dark:hover:bg-gray-800/70">
                                                        ...
                                                        <td class="px-3 py-2 text-sm"> {{ $tarea->descripcion ?: '—' }} </td>
                ...
                @empty 
                    <tr>
                        <td colspan="7" class="py-6 text-center text-gray-500"> No hay obligaciones registradas todavía. </td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div x-cloak x-show="openSidebar" x-transition.opacity class="fixed inset-0 z-40 flex justify-end bg-black/40 "> 
        ...
        {{-- Panel lateral --}}
        <div x-show="openSidebar" x-transition class="w-full max-w-md h-full bg-white dark:bg-gray-900 shadow-xl border-l flex flex-col">
            ...
            <div class="flex-1 overflow-y-auto px-4 py-4 space-y-4">
                ...
                @if (in_array($sidebarModo, ['crear_obligacion', 'editar_obligacion'])) 
                    ...
                    <div> 
                        ...
                        <input type="text"
                            wire:model.defer="formObligacion.nombre"
                    ...
                    @unless ($esUnicaSidebar)
                        <div class="grid grid-cols-3 gap-3">
                            ...
                            <div> 
                                ...
                                <input type="number"
                                    wire:model.defer="formObligacion.desfase_meses"
                                    class="w-full px-2 py-2 border rounded dark:bg-gray-700 dark:text-white"> 
                            </div>
                            <div> 
                                <label class="block text-sm mb-1">Día corte</label> 
                                <input type="number"
                                    wire:model.defer="formObligacion.dia_corte"
                                    class="w-full px-2 py-2 border rounded dark:bg-gray-700 dark:text-white"> 
                            </div>
                        </div>
                    @endunless 
            ...
            <div class="px-4 py-3 border-t flex justify-end space-x-3"> 
                ...
                {{-- Guardar --}}
                @if (in_array($sidebarModo, ['crear_obligacion', 'editar_obligacion']))
                    <button wire:click="guardarObligacion" class="px-4 py-2 bg-amber-600 text-white rounded"> Guardar </button>
                @endif

                @if (in_array($sidebarModo, ['crear_tarea', 'editar_tarea']))
                    <button wire:click="guardarTarea" class="px-4 py-2 bg-amber-600 text-white rounded"> Guardar </button>
                @endif
            </div>
        </div>
    </div>
