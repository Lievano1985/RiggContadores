<div class="p-6 bg-white dark:bg-gray-900 text-gray-900 dark:text-white">
    <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold text-stone-600 dark:text-white">Gestion de Despachos</h2>
            ...
            <div class="bg-white dark:bg-gray-900 rounded-lg w-full max-w-lg shadow-lg text-gray-900 dark:text-white max-h-[90vh] overflow-y-auto p-6">
                ...
                <form wire:submit.prevent="guardar" class="space-y-4">
                    <div>
                        ...
                        <input type="text" wire:model.defer="nombre" class="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:text-white focus:border-amber-600 focus:ring focus:ring-amber-500/40 focus:outline-none">
                        @error('nombre') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block text-sm mb-1">RFC</label>
                        <input type="text" wire:model.defer="rfc" class="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:text-white focus:border-amber-600 focus:ring focus:ring-amber-500/40 focus:outline-none">
                        @error('rfc') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>

                   {{--  <div>
                        <label class="block text-sm mb-1">Correo de contacto</label>
                        <input type="email" wire:model.defer="correo_contacto" class="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:text-white focus:border-amber-600 focus:ring focus:ring-amber-500/40 focus:outline-none">
                        @error('correo_contacto') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block text-sm mb-1">Telefono</label>
                        <input type="text" wire:model.defer="telefono_contacto" class="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:text-white focus:border-amber-600 focus:ring focus:ring-amber-500/40 focus:outline-none">
                  ...
                  {{--   <div>
                        ...
                        <input type="text" wire:model.defer="drive_folder_id" class="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:text-white focus:border-amber-600 focus:ring focus:ring-amber-500/40 focus:outline-none">
                    ...
                    @if (!$isEdit)
                        ...
                        <h4 class="text-md font-semibold text-stone-600 dark:text-white">Administrador del despacho</h4>

                        <div>
                            <label class="block text-sm mb-1">Nombre</label>
                            <input type="text" wire:model.defer="admin_name" class="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:text-white focus:border-amber-600 focus:ring focus:ring-amber-500/40 focus:outline-none">
                            @error('admin_name') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block text-sm mb-1">Correo</label>
                            <input type="email" wire:model.defer="admin_email" class="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:text-white focus:border-amber-600 focus:ring focus:ring-amber-500/40 focus:outline-none">
                            @error('admin_email') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block text-sm mb-1">Contrasena</label>
                            <input type="password" wire:model.defer="admin_password" class="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:text-white focus:border-amber-600 focus:ring focus:ring-amber-500/40 focus:outline-none">
                            @error('admin_password') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>
                    @else
                        <hr class="my-4 border-gray-300 dark:border-gray-600" />
                        <h4 class="text-md font-semibold text-stone-600 dark:text-white">Administrador asignado</h4>
    ...
    @if ($confirmingDelete)
        <div class="fixed inset-0 flex items-center justify-center bg-stone-600 bg-opacity-50 z-50">
            <div class="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-lg max-w-md w-full text-gray-900 dark:text-white">
                <h3 class="text-lg font-semibold mb-4 text-stone-600 dark:text-white">Eliminar Despacho?</h3>
